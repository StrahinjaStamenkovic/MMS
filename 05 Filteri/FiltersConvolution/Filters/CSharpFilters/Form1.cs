...
namespace CSharpFilters
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		private System.Windows.Forms.MenuItem ResizeMI;
		...
		private void InitializeComponent()
		{
            ...
            this.ResizeMI = new System.Windows.Forms.MenuItem();
            ...
            // ResizeMI
            // 
            this.ResizeMI.Index = 0;
            this.ResizeMI.Text = "Resize";
            this.ResizeMI.Click += new System.EventHandler(this.OnResize);
            // 
            // menuItem8
            // 
		...
		private void Filter_Invert(object sender, System.EventArgs e)
		{
            ...
            if (BitmapFilterBasic.Invert(m_Bitmap))
				this.Invalidate();
		}
		...
		{
            ProcessImage(BitmapFilterBasic.InvertH, m_Bitmap);
		}
		...
		private void Filter_GrayScale(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
                if (BitmapFilterBasic.GrayScale(m_Bitmap, dlg.red, dlg.green, dlg.blue))
					this.Invalidate();
			}
		...
		private void Filter_Brightness(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
                if (BitmapFilterBasic.Brightness(m_Bitmap, dlg.nValue))
					this.Invalidate();
			}
		}
		...
		private void Filter_Contrast(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
                if (BitmapFilterBasic.Contrast(m_Bitmap, (sbyte)dlg.nValue))
					this.Invalidate();
			}
		...
		private void Filter_Gamma(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
                if (BitmapFilterBasic.Gamma(m_Bitmap, dlg.red, dlg.green, dlg.blue))
					this.Invalidate();
			}
		}
		...
		private void Filter_Color(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
                if (BitmapFilterBasic.Color(m_Bitmap, dlg.red, dlg.green, dlg.blue))
					this.Invalidate();
			}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
			if(BitmapFilterConvolution.Smooth(m_Bitmap, 1))
				this.Invalidate();
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
			if(BitmapFilterConvolution.GaussianBlur(m_Bitmap, 4))
				this.Invalidate();
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
			if(BitmapFilterConvolution.MeanRemoval(m_Bitmap, 9))
				this.Invalidate();
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
			if(BitmapFilterConvolution.Sharpen(m_Bitmap, 11))
				this.Invalidate();
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
			if(BitmapFilterConvolution.EmbossLaplacian(m_Bitmap))
				this.Invalidate();
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
			if(BitmapFilterConvolution.EdgeDetectQuick(m_Bitmap))
				this.Invalidate();
		}
		...
		private void OnFilterCustom(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
				if(BitmapFilterConvolution.Conv3x3(m_Bitmap, dlg.Matrix))
					this.Invalidate();
			}
		...
		private void OnEdgeKirsh(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
				if(BitmapFilterConvolution.EdgeDetectConvolution(m_Bitmap, BitmapFilterConvolution.EDGE_DETECT_KIRSH,  (byte)dlg.nValue))
					this.Invalidate();
			}		
		}
		...
		private void OnEdgePrewitt(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
				if(BitmapFilterConvolution.EdgeDetectConvolution(m_Bitmap, BitmapFilterConvolution.EDGE_DETECT_PREWITT,  (byte)dlg.nValue))
					this.Invalidate();
			}		
		}
		...
		private void OnEdgeSobell(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
				if(BitmapFilterConvolution.EdgeDetectConvolution(m_Bitmap, BitmapFilterConvolution.EDGE_DETECT_SOBEL,  (byte)dlg.nValue))
					this.Invalidate();
			}		
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
			if(BitmapFilterConvolution.EdgeDetectHorizontal(m_Bitmap))
				this.Invalidate();
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
			if(BitmapFilterConvolution.EdgeDetectVertical(m_Bitmap))
				this.Invalidate();
		}
		...
		private void OnEdgeHomogenity(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
				if(BitmapFilterConvolution.EdgeDetectHomogenity(m_Bitmap, (byte)dlg.nValue))
					this.Invalidate();
			}		
		}
		...
		private void OnEdgeDifference(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
				if(BitmapFilterConvolution.EdgeDetectDifference(m_Bitmap, (byte)dlg.nValue))
					this.Invalidate();
			}		
		}
		...
		private void EdgeEnhance_Click(object sender, System.EventArgs e)
		{
			...
			{
				m_Undo = (Bitmap)m_Bitmap.Clone();
				if(BitmapFilterConvolution.EdgeEnhance(m_Bitmap, (byte)dlg.nValue))
					this.Invalidate();
			}		
		}
		...
		private void OnResize(object sender, System.EventArgs e)
		{
			...
			if (DialogResult.OK == dlg.ShowDialog())
			{
				m_Bitmap = BitmapFilterConvolution.Resize(m_Bitmap, dlg.nWidth, dlg.nHeight, dlg.bBilinear);
				this.Invalidate();
			}
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Flip(m_Bitmap, true, false))
				this.Invalidate();
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Flip(m_Bitmap, false, true))
				this.Invalidate();
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Flip(m_Bitmap, true, true))
				this.Invalidate();		
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.RandomJitter(m_Bitmap, 5))
				this.Invalidate();		
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Swirl(m_Bitmap, .04, false))
				this.Invalidate();					
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Swirl(m_Bitmap, .04, true))
				this.Invalidate();							
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Sphere(m_Bitmap, false))
				this.Invalidate();							
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Sphere(m_Bitmap, true))
				this.Invalidate();							
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.TimeWarp(m_Bitmap, 15, false))
				this.Invalidate();							
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.TimeWarp(m_Bitmap, 15, true))
				this.Invalidate();							
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Moire(m_Bitmap, 3))
				this.Invalidate();							
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Water(m_Bitmap, 15, false))
				this.Invalidate();							
		}
		...
		{
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Pixelate(m_Bitmap, 15, false))
				this.Invalidate();									
		}
		...
		{            
			m_Undo = (Bitmap)m_Bitmap.Clone();
            if (BitmapFilterDisplacement.Pixelate(m_Bitmap, 15, true))
				this.Invalidate();									
		}
        ...
        {
            m_Undo = (Bitmap)m_Bitmap.Clone();
            BitmapFilterBasic.PrintYBitmapUnsafe(m_Bitmap);
                this.Invalidate();	
        }
